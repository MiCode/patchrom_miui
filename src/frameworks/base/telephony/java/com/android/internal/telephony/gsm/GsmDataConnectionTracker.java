...
public final class GsmDataConnectionTracker extends DataConnectionTracker {
    ...
    boolean getInternalDataEnabled() { return mInternalDataEnabled; }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    boolean getUserDataEnabled() { return mUserDataEnabled; }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    static boolean getPolicyDataEnabled() { return sPolicyDataEnabled; }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    boolean callIsMmsDataEnabled() { return isMmsDataEnabled(); }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    String callApnIdToType(int apnId) { return apnIdToType(apnId); }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    boolean callIsDataAllowed() { return isDataAllowed(); }

    @MiuiHook(MiuiHookType.NEW_CLASS)
    static class Injector {
        static boolean isDataAllowed(GsmDataConnectionTracker tracker, ApnContext apnContext) {
            if (!(tracker.getInternalDataEnabled() && tracker.getUserDataEnabled() && getPolicyDataEnabled())) {
                if (!Phone.APN_TYPE_MMS.equals(apnContext.getApnType())
                        || ! tracker.callIsMmsDataEnabled()) {
                    return false;
                }
            }
            return tracker.callIsDataAllowed();
        }

        static String getApnType(GsmDataConnectionTracker tracker, int enabled, int apnId){
            String requestedApnType;
            if (enabled == ENABLED) {
                requestedApnType = tracker.callApnIdToType(apnId);
            } else {
    ...
    private boolean isDataAllowed(ApnContext apnContext) {
        return apnContext.isReady() && Injector.isDataAllowed(this, apnContext); // miui modify
    }
    ...
    protected void onEnableApn(int apnId, int enabled) {
        mRequestedApnType = Injector.getApnType(this, enabled, apnId); //miui add
    ...
    protected void onDataSetupComplete(AsyncResult ar) {
        ...
        if (isDataSetupCompleteOk(ar)) {
            ...
            if (dcac == null) {
                ...
                Injector.onDataSetupComplete(apnContext); // miui add
    ...
    protected void onDisconnectDone(int connId, AsyncResult ar) {
        ...
        Injector.onDisconnectDone(apnContext); // miu add
        mPhone.notifyDataConnection(apnContext.getReason(), apnContext.getApnType());
