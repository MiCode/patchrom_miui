...
public class SIMRecords extends IccRecords {
    ...
    String getSpn() { return spn; }

    @MiuiHook(MiuiHookType.NEW_CLASS)
    static class Injector {
        static void updateSpnDisplayCondition(SIMRecords records) {
            if (!android.text.TextUtils.isEmpty(records.getSpn())) {
                records.spnDisplayCondition = 0;
            }
        }

        static boolean isMatchingOperator(SIMRecords records, String plmn, String operator) {
            if (plmn.equals(operator)) {
                return true;
            }
            if (MiuiSpnOverride.getInstance().getEquivalentOperatorNumeric(plmn)
                    .equals(MiuiSpnOverride.getInstance().getEquivalentOperatorNumeric(records.getOperatorNumeric()))) {
                return true;
            }
            return false;
        }
    ...
    private boolean isOnMatchingPlmn(String plmn) {
        ...
        if (Injector.isMatchingOperator(this, plmn, getOperatorNumeric())) { // miui modify
            return true;
        }
