        ...
        if ((smsHeader == null) || (smsHeader.concatRef == null)) {
            ...
            if (smsHeader != null && smsHeader.portAddrs != null) {
                if (smsHeader.portAddrs.destPort == SmsHeader.PORT_WAP_PUSH) {
                    ...
                    return mWapPush.dispatchWapPdu(sms.getUserData(), sms.getOriginatingAddress()); // miui modify
                } else {
    ...
    protected int processMessagePart(byte[] pdu, String address, int referenceNumber,
        ...
        if (isCdmaWapPush) {
            ...
            if (destPort == SmsHeader.PORT_WAP_PUSH) {
                // Handle the PUSH
                return mWapPush.dispatchWapPdu(datagram, address); // miui modify
            } else {
                pdus = new byte[1][];
                pdus[0] = datagram;
        ...
        if (destPort != -1) {
            if (destPort == SmsHeader.PORT_WAP_PUSH) {
                ...
                // Handle the PUSH
                return mWapPush.dispatchWapPdu(output.toByteArray(), address); // miui modify
            } else {
