...
public class CallerInfo {
    ...
    static class Injector {
        ...
        static void setContactRef(Uri contactRef) { mContactRef = contactRef; }
        static int getColumnIndex(Cursor cursor, String defaultName) {
            return miui.telephony.CallerInfo.getColumnIndex(mContactRef, defaultName, cursor);
        }
    }
    ...
    public static CallerInfo getCallerInfo(Context context, Uri contactRef, Cursor cursor) {
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                ...
                Injector.setContactRef(contactRef); // miui add
                // Look for the number
                columnIndex = Injector.getColumnIndex(cursor, PhoneLookup.NUMBER); // miui modify
                if (columnIndex != -1) {
                    info.phoneNumber = cursor.getString(columnIndex);
                }

                // Look for the normalized number
                columnIndex = Injector.getColumnIndex(cursor, PhoneLookup.NORMALIZED_NUMBER); // miui modify
                if (columnIndex != -1) {
                    info.normalizedNumber = cursor.getString(columnIndex);
                }

                // Look for the label/type combo
                columnIndex = Injector.getColumnIndex(cursor, PhoneLookup.LABEL); // miui modify
                if (columnIndex != -1) {
                    int typeColumnIndex = Injector.getColumnIndex(cursor, PhoneLookup.TYPE); // miui modify
                ...
                info.extra = ExtraCallerInfo.getExtraCallerInfo(context, info, cursor); // miui add
            }
            cursor.close();
        }
