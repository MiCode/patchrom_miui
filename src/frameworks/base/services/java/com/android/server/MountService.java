...
class MountService extends IMountService.Stub
    ...
    public int mountCIFS(String ip, String user, String password, String remotepath,
        ...
        waitForReady();
        return doMountCIFS(ip, user, password, remotepath, localpath, options);
    }
    ...
    public int unmountCIFS(String mountPoint) {
        ...
        waitForReady();
        return doUnmountCIFS(mountPoint);
    }

    private int doMountCIFS(String ip, String user, String password, String remotepath,
            String localpath, String options) {
        int rc = StorageResultCode.OperationSucceeded;

        if (DEBUG_EVENTS) Slog.i(TAG, "doMountCIFS: Mouting " + localpath);
        if (ip == null || localpath == null) {
            Slog.e(TAG, "invalid argument");
            return StorageResultCode.OperationFailedInternalError;
        }
        String extraOption = new String("unc=\\\\"+ ip + "\\" + remotepath + ",");
        if (user != null) {
            extraOption = new String(extraOption + "username=" + user + ",");
        } else extraOption = new String(extraOption + "username=guest,");
        if (password != null) {
            extraOption = new String(extraOption + "password=" + password + ",");
        }
        if (options != null) {
            extraOption = new String(extraOption + options);
        }
        if (extraOption.endsWith(",")) {
            extraOption.substring(0, extraOption.length() - 1);
        }
        try {
                mConnector.execute("cifs", "mount", "none", localpath, extraOption);
        } catch (NativeDaemonConnectorException e) {
            /*
             * Mount failed for some reason
             */
    ...
    private int doUnmountCIFS(String mountPoint) {
        if (mountPoint == null) {
            Slog.e(TAG, "invalid argument");
            return StorageResultCode.OperationFailedInternalError;
        }
        try {
            mConnector.execute("cifs", "unmount", mountPoint);
        } catch (NativeDaemonConnectorException e) {
            int code = e.getCode();
            if (code == VoldResponseCode.OpFailedVolNotMounted) {
                return StorageResultCode.OperationFailedStorageNotMounted;
