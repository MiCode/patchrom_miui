...
public class PowerManagerService extends IPowerManager.Stub
    ...
    boolean getProximitySensorActive() { return mProximitySensorActive; }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    int getPowerState() { return mPowerState; }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    void setProxIgnoredBecauseScreenTurnedOff(boolean value) { mProxIgnoredBecauseScreenTurnedOff = value; }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    void callGoToSleepLocked(long time, int reason) { goToSleepLocked(time, reason); }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    Handler getScreenBrightnessHandler() { return mScreenBrightnessHandler; }

    @MiuiHook(MiuiHookType.NEW_CLASS)
    static class Injector {
        static boolean FALSE = false;

        static void sleepIfProximitySensorActive(PowerManagerService service) {
            if (service.getProximitySensorActive() && (service.getPowerState() & SCREEN_ON_BIT) != 0) {
                service.callGoToSleepLocked(SystemClock.uptimeMillis(),
                        WindowManagerPolicy.OFF_BECAUSE_OF_PROX_SENSOR);
                service.setProxIgnoredBecauseScreenTurnedOff(false);
            }
        }

        static void animateTo(PowerManagerService service, ScreenBrightnessAnimator animator, int target, int mask, int animationDuration) {
            synchronized (animator) {
                animator.currentValue = animator.endValue = target;
            }
            service.getScreenBrightnessHandler().removeCallbacksAndMessages(null);
        }
    }
    ...
    static final int AUTODIMNESS_ANIM_STEPS = 2 * ANIM_STEPS;
            ...
            if ((flags & LOCK_MASK) == PowerManager.PROXIMITY_SCREEN_OFF_WAKE_LOCK) {
                ...
                if ((wl.flags & PowerManager.ACQUIRE_CAUSES_WAKEUP) != 0) {
                    ...
                    if ((mWakeLockState & SCREEN_ON_BIT) != 0
                        ...
                        // mProximitySensorActivie = false; // miui delete
                    }
    ...
    public void preventScreenOn(boolean prevent) {
        ...
        synchronized (mLocks) {
            if (prevent) {
                ...
                Injector.sleepIfProximitySensorActive(this); // miui add
    ...
    private int setScreenStateLocked(boolean on) {
        ...
        if (on) {
            if (mInitialized && ((mPowerState & SCREEN_ON_BIT) == 0 || mSkippedScreenOn)) {
                ...
                Injector.animateTo(this, mScreenBrightnessAnimator, PowerManager.BRIGHTNESS_OFF, SCREEN_BRIGHT_BIT, 0); // miui add
            }
        }
    ...
    private void setPowerState(int newState, boolean noChangeLights, int reason)
    {
        synchronized (mLocks) {
            ...
            if (mProximitySensorActive) {
                ...
                // newState = (newState & ~SCREEN_BRIGHT); // miui delete
            }
    ...
    private void userActivity(long time, long timeoutOverride, boolean noChangeLights,
        ...
        synchronized (mLocks) {
            ...
            if (mProximitySensorActive && mProximityWakeLockCount == 0) {
                // mProximitySensorActive = false; // miui delete
            }

            if (mLastEventTime <= time || force) {
                mLastEventTime = time;
                if ((mUserActivityAllowed && !Injector.FALSE) || force) { // miui modify
