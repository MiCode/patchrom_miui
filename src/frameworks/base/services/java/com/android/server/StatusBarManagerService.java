    ...
    implements WindowManagerService.OnHardKeyboardStatusChangeListener
{
    ...
    ArrayList<StatusRecord> mStatusRecords = new ArrayList<StatusRecord>();
    ...
    private class StatusRecord implements IBinder.DeathRecipient {
        String pkg;
        IBinder token;
    ...
    void manageStatusListLocked(int what, IBinder token, String pkg) {
        if (SPEW) {
            Slog.d(TAG, "manageStatusListLocked what=0x" + Integer.toHexString(what) + " pkg=" + pkg);
        }
        // update the list
        final int N = mStatusRecords.size();
        StatusRecord tok = null;
        int i;
        for (i=0; i<N; i++) {
            StatusRecord t = mStatusRecords.get(i);
            if (t.token == token) {
                tok = t;
                break;
            }
    ...
    public void setStatus(int what, IBinder token, String action, Bundle ext) {
        synchronized(mLock) {
            ...
            if (mBar != null) {
                try {
                    mBar.setStatus(what, action, ext);
                } catch (RemoteException ex) {
                }
            }
        }
