...
public final class AttributeCache {
    ...
    static class Injector {
        static Package getPackage(WeakHashMap<String, WeakReference<Package>> packages, String packageName) {
            WeakReference<Package> ref = packages.get(packageName);
            if (ref != null) return ref.get();
            return null;
        }

        static void putPackage(WeakHashMap<String, WeakReference<Package>> packages, String packageName, Package pkg) {
            packages.put(packageName, new WeakReference<Package>(pkg));
        }
    }
    ...
    private final WeakHashMap<String, WeakReference<Package>> mPackages =
            new WeakHashMap<String, WeakReference<Package>>();
    ...
    public Entry get(String packageName, int resId, int[] styleable) {
        synchronized (this) {
            Package pkg = Injector.getPackage(mPackages, packageName); // miui modify
            ...
            if (pkg != null) {
                ...
                Injector.putPackage(mPackages, packageName, pkg); // miui modify
            }
