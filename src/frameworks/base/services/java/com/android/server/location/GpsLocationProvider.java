...
public class GpsLocationProvider implements LocationProviderInterface {
    ...
    boolean getNavigating() { return mNavigating; }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    SparseIntArray getClientUids() { return mClientUids; }

    @MiuiHook(MiuiHookType.NEW_CLASS)
    static class Injector {
        static void appendUidExtra(GpsLocationProvider provider, Intent intent) {
            if (provider.getNavigating() && provider.getClientUids().size() > 0) {
                intent.putExtra(Intent.EXTRA_UID, provider.getClientUids().keyAt(0));
            }
        }
    }
    ...
    private void reportStatus(int status) {
        ...
        synchronized(mListeners) {
            ...
            if (wasNavigating != mNavigating) {
                ...
                Injector.appendUidExtra(this, intent); // miui add
                mContext.sendBroadcast(intent);
            }
        }
    }
