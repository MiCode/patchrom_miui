...
public class MediaScanner
{
    ...
    static class Injector {
        static void setAllSettingsIfNotSet(MediaScanner scanner, FileEntry entry, boolean ringtones, boolean notifications, boolean alarms) {
            if (notifications) {
                setSettingIfNotSet(scanner, Settings.System.NOTIFICATION_SOUND, entry.mPath);
                setSettingIfNotSet(scanner, ExtraSettings.System.SMS_DELIVERED_SOUND, entry.mPath);
                setSettingIfNotSet(scanner, ExtraSettings.System.SMS_RECEIVED_SOUND, entry.mPath);
            } else if (ringtones) {
                setSettingIfNotSet(scanner, Settings.System.RINGTONE, entry.mPath);
            } else if (alarms) {
                setSettingIfNotSet(scanner, Settings.System.ALARM_ALERT, entry.mPath);
            }
        }

        static void setSettingIfNotSet(MediaScanner scanner, String settingName, String path) {
...
//            Log.v(TAG, "scanFile: " + path + " took " + (t2-t1));
        ...
        private Uri endFile(FileEntry entry, boolean ringtones, boolean notifications,
            ...
            if(needToSetSettings) {
                Injector.setAllSettingsIfNotSet(MediaScanner.this, entry, ringtones, notifications, alarms); // miui add
                if (notifications) {
