...
public class AudioManager {
    ...
    static class Injector {
        static boolean checkQuietModeForNotification(Context context, int streamType) {
            return (streamType == STREAM_NOTIFICATION
                    && ExtraSettings.AntiSpam.isQuietModeEnable(context)) ? true : false;
        }
    }
    // END
    ...
    public int getStreamVolume(int streamType) {
        // MIUI ADD:
        if (Injector.checkQuietModeForNotification(mContext, streamType)) return 0;
        IAudioService service = getService();
        try {
            if (mUseMasterVolume) {
