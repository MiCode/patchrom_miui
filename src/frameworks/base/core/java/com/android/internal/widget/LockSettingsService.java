...
public class LockSettingsService extends ILockSettings.Stub {
    ...
    static class Injector {
         ...
         */
        static byte[] passwordToHash(byte[] stored, byte[] password) {
            if (password == null || stored.length == 72) {
                return password;
            }
            byte[] hashed = null;
            try {
                 hashed = MessageDigest.getInstance("MD5").digest(password);
    ...
    public boolean checkPassword(byte[] hash, int userId) throws RemoteException {
        ...
        try {
            ...
            return Arrays.equals(stored, Injector.passwordToHash(stored, hash)); // miui modify
        } catch (FileNotFoundException fnfe) {
            Slog.e(TAG, "Cannot read file " + fnfe);
            return true;
        } catch (IOException ioe) {
