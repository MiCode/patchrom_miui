...
public final class BluetoothAdapter {
     ...
     * @hide
     */
    public static class ProfileProxyHandler {
        private final BluetoothAdapter mAdapter;
        private final int mProfile;
        private final BluetoothProfile mProxy;

        ProfileProxyHandler(BluetoothAdapter adapter, int profile, BluetoothProfile proxy) {
            mAdapter = adapter;
            mProfile = profile;
            mProxy  = proxy;
        }
        ...
        public void close() {
            mAdapter.closeProfileProxy(mProfile, mProxy);
        }
    }
    // END
     ...
     * @hide
     */
    public ProfileProxyHandler getProfileProxy(Context context, BluetoothProfile.ServiceListener listener,
            int profile, Object nullObject) {
        if (context == null || listener == null) return null;

        if (profile == BluetoothProfile.HEADSET) {
            BluetoothHeadset headset = new BluetoothHeadset(context, listener);
            return new ProfileProxyHandler(this, profile, headset);
        } else if (profile == BluetoothProfile.A2DP) {
            BluetoothA2dp a2dp = new BluetoothA2dp(context, listener);
            return new ProfileProxyHandler(this, profile, a2dp);
        } else if (profile == BluetoothProfile.INPUT_DEVICE) {
            BluetoothInputDevice iDev = new BluetoothInputDevice(context, listener);
            return new ProfileProxyHandler(this, profile, iDev);
        } else if (profile == BluetoothProfile.PAN) {
            BluetoothPan pan = new BluetoothPan(context, listener);
            return new ProfileProxyHandler(this, profile, pan);
        } else if (profile == BluetoothProfile.HEALTH) {
            BluetoothHealth health = new BluetoothHealth(context, listener);
            return new ProfileProxyHandler(this, profile, health);
        } else {
            return null;
        }
    }
