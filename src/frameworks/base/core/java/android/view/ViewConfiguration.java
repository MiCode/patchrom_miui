...
public class ViewConfiguration {
    static class Injector {
        static final SparseArray<ViewConfiguration> sV5Configrations =
        ...
        static ViewConfiguration get(Context context) {
            if (UiUtils.isV5Ui(context)) {
                ...
                ViewConfiguration configuration = sV5Configrations.get(density);
                if (configuration == null) {
                    configuration = getInstance(context);
                    sV5Configrations.put(density, configuration);
                }
        ...
        static int getOverScrollDistance(Context context, int defaultValue) {
            int id = UiUtils.resolveAttribute(context, miui.R.attr.v5_over_scroll_distance);
            if (id > 0) {
                return (int) (context.getResources().getDimension(id) + 0.5f);
            }
            return defaultValue;
        }

        static int getOverFlingDistance(Context context, int defaultValue) {
            int id = UiUtils.resolveAttribute(context, miui.R.attr.v5_over_fling_distance);
            if (id > 0) {
                return (int) (context.getResources().getDimension(id) + 0.5f);
            }
            return defaultValue;
    ...
    private ViewConfiguration(Context context) {
        ...
        mOverscrollDistance = Injector.getOverScrollDistance(context, (int) (sizeAndDensity * OVERSCROLL_DISTANCE + 0.5f)); // Miui Hook
        mOverflingDistance = Injector.getOverFlingDistance(context, (int) (sizeAndDensity * OVERFLING_DISTANCE + 0.5f)); // Miui Hook
     ...
     * MIUI ADD
     */
    public static ViewConfiguration getInstance(Context context) {
        return new ViewConfiguration(context);
    }
    ...
    public static ViewConfiguration get(Context context) {
        // Miui Hook
        ViewConfiguration cfg = Injector.get(context);
        if (cfg != null) {
            return cfg;
        }
