...
public abstract class LayoutInflater {
    ...
    static class Injector {
        static final int[] sLayoutPairs = new int[] {
            android.R.layout.simple_list_item_1, miui.R.layout.v5_simple_list_item_1_dialog,
            miui.R.layout.android_simple_list_item_2_single_choice, miui.R.layout.v5_simple_list_item_2_single_choice_dialog,
            android.R.layout.simple_spinner_dropdown_item, miui.R.layout.v5_select_dialog_singlechoice,
            miui.R.layout.android_expanded_menu_layout, miui.R.attr.v5_expanded_menu_layout,
            miui.R.layout.android_list_menu_item_layout, miui.R.attr.v5_popup_menu_item_layout,
        ...
        static boolean needResolveReference(int layoutId) {
            return (layoutId & 0xff000000) != 0x01000000;
        }

        static boolean isMiuiAttribute(int layoutId) {
            return (layoutId & 0x00ff0000) == (miui.R.attr.miui_ui_version & 0x00ff0000);
        }

        static void buildLayoutMap(Context context) {
            if (sLayoutMap != null) {
                return;
            }
            ...
            synchronized (Injector.class) {
                if (sLayoutMap == null) {
                    sLayoutMap = new SparseArray<Integer>();
                    for (int i = 0; i < sLayoutPairs.length; i += 2) {
                        int index = needResolveReference(sLayoutPairs[i]) ?
                                miui.util.ResourceMapper.resolveReference(context, sLayoutPairs[i]) : sLayoutPairs[i];
                        sLayoutMap.put(index, sLayoutPairs[i + 1]);
                    }
                }
            }
        }
        ...
        static int getResourceId(Context context, int resource) {
            ...
            if (UiUtils.isV5Ui(context)) {
                ...
                Integer layout = sLayoutMap.get(resource);
                if (layout != null) {
                    newResource = layout.intValue();
                    if (isMiuiAttribute(newResource)) {
                        newResource = UiUtils.resolveAttribute(context, newResource);
                        if (newResource <= 0) {
    ...
    public View inflate(int resource, ViewGroup root, boolean attachToRoot) {
        ...
        // Miui Hook
        resource = Injector.getResourceId(getContext(), resource);
