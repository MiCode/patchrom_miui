...
public class CursorWindow extends SQLiteClosable implements Parcelable {
    ...
    public static class Injector {
        ...
        final static String EXCEPTION_MESSAGE_FORMAT =
                "Process %d exceeded cursor quota %d, will kill it";
        final static String EXCEPTION_MESSAGE_REGEX =
                "^Process \\d+ exceeded cursor quota \\d+, will kill it$";

        static SparseIntArray sPidCounts = new SparseIntArray();
        ...
        static void addQuota(int pid) {
            checkQuota(pid, true);
        }

        static void checkQuota(int pid, boolean add) {
            synchronized (sPidCounts) {
                int count = sPidCounts.get(pid);
                if (add) {
                    ++count;
                    ...
                    if (count >= 0) {
                        ...
                        sPidCounts.delete(pid);
                    }
                }
            }
        }

        public static boolean causeByCursorLeak(String msg) {
            if (msg != null && msg.length() > 0) {
                return msg.matches(EXCEPTION_MESSAGE_REGEX);
            } else {
                return false;
            }
        }
    ...
    private void recordClosingOfWindow(int window) {
        synchronized (sWindowToPidMap) {
            ...
            // MIUI ADD:
            Injector.delQuota(sWindowToPidMap, window);
