...
public abstract class SensorManager {
    ...
    public static float getAltitude(float p0, float p) {
        ...
        p0 = Injector.getSeaLevelPressure(p0);// Miui hook
        return 44330.0f * (1.0f - (float)Math.pow(p/p0, coef));
    }

    @MiuiHook(MiuiHookType.NEW_CLASS)
    private static class Injector {
        static float getSeaLevelPressure(float p0) {
            if (p0 - PRESSURE_STANDARD_ATMOSPHERE < 0.000001
                    && p0 - PRESSURE_STANDARD_ATMOSPHERE > -0.000001) {
                String pressure = SystemProperties.get(ExtraSettings.System.LOCAL_SEA_LEVEL_PRESSURE);
                if (!TextUtils.isEmpty(pressure)) {
                    try {
                        p0 = Float.parseFloat(pressure);
                    } catch (NumberFormatException e) {
                        Log.e("getAltitude",
                                "System property local sea level pressure not a float value");
                    }
                }
            }
            return p0;
