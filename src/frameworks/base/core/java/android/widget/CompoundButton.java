...
public abstract class CompoundButton extends Button implements Checkable {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (buttonDrawable == null || !UiUtils.isV5Ui(getContext())) {
            return;
        }
        ...
        if (widthMode != MeasureSpec.EXACTLY) {
            width = widthSize + buttonDrawable.getIntrinsicWidth();
        }
    ...
    @android.annotation.MiuiHook(android.annotation.MiuiHook.MiuiHookType.NEW_METHOD)
    boolean onDrawMiui(Canvas canvas) {
        final Drawable buttonDrawable = mButtonDrawable;
        if (buttonDrawable == null || !UiUtils.isV5Ui(getContext())) {
            return false;
        }
        ...
        if (buttonDrawable.getCurrent() instanceof NinePatchDrawable) {
            height = getHeight() - y - getPaddingBottom();
        } else {
        ...
        buttonDrawable.setBounds(x, y, x + buttonDrawable.getIntrinsicWidth(), y + height);
        buttonDrawable.draw(canvas);

        canvas.save();
        canvas.translate(buttonDrawable.getIntrinsicWidth(), 0);
        super.onDraw(canvas);
        canvas.restore();
        return true;
    }
    ...
    protected void onDraw(Canvas canvas) {
        if (onDrawMiui(canvas)) { // Miui Hook
            return;
        }
