...
public class SearchView extends LinearLayout implements CollapsibleActionView {
    ...
    private void updateSubmitArea() {
        ...
        miuiUpdateSubmitArea(); // Miui Hook
    }
    ...
    private boolean miuiUpdateFocusedState() {
        boolean retval = false;
        if (UiUtils.isV5Ui(mContext)) {
            boolean focused = mQueryTextView.hasFocus();
            mSearchEditFrame.getBackground().setState(focused ? FOCUSED_STATE_SET : EMPTY_STATE_SET);
            mSubmitArea.getBackground().setState(focused ? FOCUSED_STATE_SET : EMPTY_STATE_SET);
            invalidate();
            retval = true;
        }
    ...
    @android.annotation.MiuiHook(android.annotation.MiuiHook.MiuiHookType.NEW_METHOD)
    private void miuiUpdateSubmitArea() {
        if (!UiUtils.isV5Ui(mContext)) {
            return;
        }

        final int leftPadding = mSearchEditFrame.getPaddingLeft();
        final int topPadding = mSearchEditFrame.getPaddingTop();
        Rect paddingRect = new Rect();
        mSearchEditFrame.getBackground().getPadding(paddingRect);
        final int rightPadding = mSubmitArea.getVisibility() == VISIBLE ? 0 : paddingRect.right;
        final int bottomPadding = mSearchEditFrame.getPaddingBottom();
        mSearchEditFrame.setPadding(leftPadding, topPadding, rightPadding, bottomPadding);
    }
}
