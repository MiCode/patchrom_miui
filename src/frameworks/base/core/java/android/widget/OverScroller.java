...
public class OverScroller {
        ...
        switch (mMode) {
            ...
            case FLING_MODE:
                if (!mScrollerX.mFinished) {
                    ...
                    if (!mScrollerX.update(false)) {
                        // MIUI MODIFY
                        // Original:
                        // if (!mScrollerX.continueWhenFinished())
                        if (!mScrollerX.continueWhenFinished(mSpringOverScrollEnable)) {
                            mScrollerX.finish();
                        }
                    }
                }
                ...
                if (!mScrollerY.mFinished) {
                    ...
                    if (!mScrollerY.update(false)) {
                        // MIUI MODIFY
                        // Original:
                        // if (!mScrollerY.continueWhenFinished())
                        if (!mScrollerY.continueWhenFinished(mSpringOverScrollEnable)) {
                            mScrollerY.finish();
                        }
                    }
                }
    ...
    static class SplineOverScroller {
        ...
        // Current deceleration scale when overscroll
        @android.annotation.MiuiHook(android.annotation.MiuiHook.MiuiHookType.NEW_FIELD)
        private float mDecelerationScale = 1.0f;
        ...
        private void startSpringback(int start, int end, int velocity) {
            ...
            mDeceleration = getDeceleration(delta) * mDecelerationScale;
        ...
        boolean continueWhenFinished(boolean springOverScrollEnable) {
            boolean force = false;
            switch (mState) {
                case SPLINE:
                    // Duration from start to null velocity
                ...
                case BALLISTIC:
                    if (springOverScrollEnable) {
                        float t = (float) ((float)Math.abs(mOver) / ((float) mTotalOverDistance));
                        final float scale = (float) (Math.pow(t, 3) * 20.0f) + 1.0f;
                        mDecelerationScale = scale;
                        force = true;
                    }
                    mStartTime += mDuration;
                    startSpringback(mFinal, mStart, 0);
        ...
        @android.annotation.MiuiHook(android.annotation.MiuiHook.MiuiHookType.NEW_METHOD)
        void setDecelerationScale(float scale) {
            mDecelerationScale = scale;
        }

        @android.annotation.MiuiHook(android.annotation.MiuiHook.MiuiHookType.NEW_METHOD)
        void setTotalOverDistance(int distance) {
            mTotalOverDistance = distance;
        }
    }
     ...
     */
    @android.annotation.MiuiHook(android.annotation.MiuiHook.MiuiHookType.NEW_METHOD)
    public final void setDecelerationScale(float scale) {
        mScrollerX.setDecelerationScale(scale);
        mScrollerY.setDecelerationScale(scale);
    }
     ...
     */
    @android.annotation.MiuiHook(android.annotation.MiuiHook.MiuiHookType.NEW_METHOD)
    public final void setTotalOverDistance(int distance) {
        mScrollerX.setTotalOverDistance(distance);
        mScrollerY.setTotalOverDistance(distance);
    }
     ...
     */
    @android.annotation.MiuiHook(android.annotation.MiuiHook.MiuiHookType.NEW_METHOD)
    public void setSpringOverScrollEnable(boolean enable) {
        mSpringOverScrollEnable = enable;
    }
    ...
    public final boolean checkSpringBackState() {
        return (mScrollerY.mState == SplineOverScroller.CUBIC) || (mScrollerX.mState == SplineOverScroller.CUBIC);
    }
}
