...
public class Editor {
    ...
    long getLastCutOrCopyTime() {
        return mTextView.LAST_CUT_OR_COPY_TIME;
    }
    ...
    boolean startTextSelectionMode() {
        if (!mIsInTextSelectionMode) {
            if (!hasSelectionController()) {
                Log.w(TAG, "TextView has no selection controller. Selection mode cancelled.");
                return false;
            }
            ...
            if (!mTextView.hasSelection()) {
                if ( !getSelectionController().selectCurrentRange() ) {
                    Log.i(TAG, "can not selection current range");
                    return false;
                }
            }
        ...
        switch (v.getId()) {
            ...
            case miui.R.id.buttonPaste:
                ...
                if (paste != null && paste.length() > 0) {
                    Selection.setSelection((Spannable) mTextView.getText(), max);
                    mTextView.replaceText_internal(min,max,paste);
                    stopTextSelectionMode();
                }
                break;
    ...
    ArrayList<Boolean> getFloatPanelShowHides(int[] buttons) {
        ...
        Boolean isTextEditable = mTextView.isTextEditable();
    ...
    boolean getButtonShowHides(boolean isTextEditable, int id) {
        try {
            switch (id) {
                ...
                case miui.R.id.buttonPaste:
                    return isTextEditable && mTextView.canPaste();
                ...
                case miui.R.id.buttonCut:
                    return isTextEditable && mTextView.canCut();
            }
            return false;
        }
    ...
    boolean needInsertPanel() {
        return !mIsInTextSelectionMode && (canSelectText() || mTextView.canPaste());
    }
