...
public interface IMountService extends IInterface {
    ...
    public static abstract class Stub extends Binder implements IMountService {
        private static class Proxy implements IMountService {
            ...
            //MIUI ADD (for miphone only)
            /*
             * add interface for CIFS mount,unmount
            */
            public int mountCIFS(String ip, String user, String password, String remotepath, String localpath, String options)
                    throws RemoteException {
                Parcel _data = Parcel.obtain();
                Parcel _reply = Parcel.obtain();
                int _result;
                try {
                    ...
                    mRemote.transact(Stub.TRANSACTION_mountCIFS, _data, _reply, 0);
                    _reply.readException();
                    _result = _reply.readInt();
                } finally {
                    _reply.recycle();
            ...
            public int unmountCIFS(String mountpoint)
                    throws RemoteException {
                Parcel _data = Parcel.obtain();
                Parcel _reply = Parcel.obtain();
                int _result;
                try {
                    _data.writeInterfaceToken(DESCRIPTOR);
                    _data.writeString(mountpoint);
                    mRemote.transact(Stub.TRANSACTION_unmountCIFS, _data, _reply, 0);
                    _reply.readException();
                    _result = _reply.readInt();
                } finally {
                    _reply.recycle();
        ...
        // MIUI ADD (for miphone only)
        static final int TRANSACTION_mountCIFS = IBinder.FIRST_CALL_TRANSACTION + 34;

        static final int TRANSACTION_unmountCIFS = IBinder.FIRST_CALL_TRANSACTION + 35;
        // END
        /**
        ...
        public boolean onTransact(int code, Parcel data, Parcel reply,
            ...
            switch (code) {
                ...
                case TRANSACTION_mountCIFS: {
                    ...
                    String remotepath;
                    remotepath = data.readString();
                    String localpath;
                    localpath = data.readString();
                    String options;
                    options = data.readString();
                    int resultCode = mountCIFS(ip, user, password, remotepath, localpath, options);
                    reply.writeNoException();
                    reply.writeInt(resultCode);
                    return true;
                }
                case TRANSACTION_unmountCIFS: {
                    data.enforceInterface(DESCRIPTOR);
                    String mountPoint;
                    mountPoint = data.readString();
                    int resultCode = unmountCIFS(mountPoint);
                    reply.writeNoException();
                    reply.writeInt(resultCode);
                    return true;
                }
    ...
    // MIUI ADD (miphone only)
    public int mountCIFS(String ip, String user, String password, String remotepath,
            String localpath, String options) throws RemoteException;

    public int unmountCIFS(String mountPoint) throws RemoteException;
    // END
}
