...
import miui.text.util.EmojiSmileys;
...
extends Layout
{
    ...
    public StaticLayout(CharSequence source, int bufstart, int bufend,
        ...
        this(source, bufstart, bufend, paint, outerwidth, OUTERHEIGHT_INFINITE, align,
                spacingmult, spacingadd, includepad, null, 0);
    }
     ...
     */
    public StaticLayout(CharSequence source, int bufstart, int bufend,
                        TextPaint paint, int outerwidth, int outerheight,
                        Alignment align,
                        float spacingmult, float spacingadd,
                        boolean includepad,
                        TextUtils.TruncateAt ellipsize, int ellipsizedWidth) {
        ...
        generate(source, bufstart, bufend, paint, outerwidth, outerheight, align,
                 spacingmult, spacingadd, includepad, includepad,
                 ellipsize != null, ellipsizedWidth, ellipsize);

        if(outerheight != OUTERHEIGHT_INFINITE && getLineTop(getLineCount()) > outerheight) {
            if(ellipsize == TextUtils.TruncateAt.END) {
                int lastline = getLineForVertical(outerheight);
                if(lastline > 0 && lastline < getLineCount() && lastline + 1 < getLineCount()) {
                    if(getLineTop(lastline + 1) > outerheight) {
                        --lastline;
                    }
                    mLines[lastline * mColumns + ELLIPSIS_START] =
                        getLineStart(lastline + 1) - getLineStart(lastline) - 1;
                    mLines[lastline * mColumns + ELLIPSIS_COUNT] = 1;
                    mLineCount = lastline + 1;
                }
            }
        }
    ...
    /* package */ void generate(CharSequence source, int bufstart, int bufend,
        ...
        generate(source, bufstart, bufend,
                paint, outerwidth, OUTERHEIGHT_INFINITE,
                align,
                spacingmult, spacingadd,
                includepad, trackpad,
                breakOnlyAtSpaces,
                ellipsizedWidth, where);
    }
                ...
                for (int j = i; j < next; j++) {
                    ...
                    if (w <= width) {
                        ...
                        if (c == ' ' || c == '\t' ||
                            ...
                            where != TextUtils.TruncateAt.END && outerheight == OUTERHEIGHT_INFINITE) {
                        if (ok != here) {
...
*/
    ...
    /** @hide */
    public static final int OUTERHEIGHT_INFINITE = 0;
