...
public class WebSettingsClassic extends WebSettings {
    ...
    public synchronized void setNightReadModeEnabled(boolean enable) {
        mNightReadMode = enable;
        postSync();
    }
    ...
    public void setProperty(String key, String value) {
        if (mWebView.nativeSetProperty(key, value)) {
            // mWebView.invalidate();
            // The previous line is changed by 4.1, but it sometimes just only
            // refreshes the top screen's graphics areas when setProperty, so
            // we have to make the full page be invalideated.
