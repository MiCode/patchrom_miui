...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    @MiuiHook(MiuiHookType.NEW_METHOD)
    Runnable getPowerLongPress() { return mPowerLongPress; }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    void setPowerLongPress(Runnable value) { mPowerLongPress = value; }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    Context getContext() { return mContext; }

    @MiuiHook(MiuiHookType.NEW_METHOD)
    void callInterceptPowerKeyUp(boolean canceled) { interceptPowerKeyUp(canceled); }

    @MiuiHook(MiuiHookType.NEW_CLASS)
    static class Injector {
        static boolean getNeedsMenuLw(WindowState focus, WindowState ws) {
            return true;
        }
        ...
        static void performReleaseHapticFeedback(PhoneWindowManager manager, KeyEvent event, int policyFlags) {
            ...
            if (! down && ((policyFlags & WindowManagerPolicy.FLAG_VIRTUAL) != 0) && (event.getRepeatCount() == 0)) {
                manager.performHapticFeedbackLw(null, HapticFeedbackConstants.VIRTUAL_RELEASED, false);
            }
        }

        static void sendPowerUpBroadcast(PhoneWindowManager manager) {
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        if (!mHeadless) {
            ...
            mKeyguardMediator = new MiuiKeyguardViewMediator(context, this, powerManager); // miui modify
        }
    ...
    public void beginLayoutLw(int displayWidth, int displayHeight, int displayRotation) {
        ...
        if (mStatusBar != null) {
            ...
            if (mStatusBar.isVisibleLw() && !mStatusBar.isAnimatingLw()) {
                ...
                // and not in the process of animating on or off, then
                // we can tell the app that it is covered by it.
                // mSystemTop = mUnrestrictedScreenTop + mStatusBarHeight; //MiuiHook
            }
        }
    }
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags, boolean isScreenOn) {
        ...
        Injector.performReleaseHapticFeedback(this, event, policyFlags); // miui add
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_POWER: {
                ...
                if (down) {
                    ...
                    Injector.sendPowerUpBroadcast(this); // miui add
                }
                break;
            }
    ...
    private int updateSystemUiVisibilityLw() {
        ...
        final boolean needsMenu = Injector.getNeedsMenuLw(mFocusedWindow, mTopFullscreenOpaqueWindowState); // miui modify
