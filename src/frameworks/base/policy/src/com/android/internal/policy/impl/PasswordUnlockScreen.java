...
public class PasswordUnlockScreen extends LinearLayout implements KeyguardScreen,
    ...
    private void initLockByFindDevice() {
        mIsLockByFindDevice = Settings.Secure.getInt(mContext.getContentResolver(),
    ...
    @MiuiHook(MiuiHookType.NEW_METHOD)
    private void clearPinLockForFindDevice(String entry) {
        if (mIsLockByFindDevice) {
            mLockPatternUtils.clearLock(false);
            Settings.Secure.putInt(mContext.getContentResolver(), ExtraSettings.Secure.FIND_DEVICE_PIN_LOCK, 0);
    ...
    private void verifyPasswordAndUnlock() {
        ...
        if (mLockPatternUtils.checkPassword(entry)) {
            ...
            clearPinLockForFindDevice(entry);
